//! Step efficiency: an LLM-judge metric measuring how efficiently the assistant
//! completed the task across the multi-turn conversation.
...
const CLASS_NAME: &str = "StepEfficiencyMetric";
const METHOD: &str = "generate_verdict";
const TEMPLATE: &str = include_str!("../../templates/step_efficiency/generate_verdict.txt");

/// StepEfficiencyMetric: scores how efficiently the assistant completed the
/// task, rewarding concise, minimal-step solutions across the conversation.
///
...
impl StepEfficiencyMetric {
    /// Start building a [`StepEfficiencyMetric`].
    pub fn builder() -> StepEfficiencyMetricBuilder {
        StepEfficiencyMetricBuilder::default()
    }
...
impl_conversational_metric!(StepEfficiencyMetric, "StepEfficiencyMetric");

/// Builder for [`StepEfficiencyMetric`].
#[derive(Debug, Default)]
pub struct StepEfficiencyMetricBuilder {
    config: MetricConfig,
    provider: Option<Provider>,
}

impl StepEfficiencyMetricBuilder {
    ...
    pub fn build(self) -> StepEfficiencyMetric {
        let provider = self
            .provider
            .expect("StepEfficiencyMetric requires an LLM provider");
...
mod tests {
    ...
    fn case() -> ConversationalTestCase {
        ConversationalTestCase::builder()
            .turn(
                Turn::builder()
                    .input("Refund my order #123.")
                    .actual_output("Your order #123 has been refunded.")
                    .build(),
            )
            .turn(
                Turn::builder()
                    .input("Confirm the amount.")
                    .actual_output("$45.99 has been refunded.")
                    .build(),
            )
            .build()
    }
    ...
    #[tokio::test]
    async fn scores_high_when_efficient() {
        let provider = MockLlmProvider::text(r#"{"score": 0.9, "reason": "good"}"#);
        let mut metric = StepEfficiencyMetric::builder()
            .provider(provider)
            .threshold(0.7)
            .build();
    ...
    #[tokio::test]
    async fn scores_low_when_inefficient() {
    ...
    async fn skips_when_no_required_fields() {
        ...
        let mut metric = StepEfficiencyMetric::builder().provider(provider).build();
